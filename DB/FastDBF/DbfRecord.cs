...
namespace IO.FastDBF
{
    ...
    public class DbfRecord
    {
        ...
        {
            string s = "";
            string[] c = _colNameToIdx.Keys.ToArray();
            string data = new string(encoding.GetChars(_data));


            DbfColumn[] acol = _header.GetColumns();
            string[] acn = acol.Select(x => x.Name).ToArray();
            var atyp = acol.Select(x => x.ColumnType).ToArray();
            int[] alen = acol.Select(x => x.Length).ToArray();
            int total = alen.Sum();
            ...
            {
                int offset = 0;
                for (int k = 0; k < alen.Length; k++)
                {
                    int len = alen[k] + (offset == 0 ? 1 : 0);
                    bf = new byte[len];
                    Buffer.BlockCopy(_data, offset, bf, 0, len);
                    offset += len;

                    if (k > 0 && k != alen.Length)
                        bi.Append(",");

                    bi.Append(@"""" + acn[k] + @""":");

                    int len32 = ComputeAdditionChecksum(bf);
                    if (len32 == 32 * len)
                    {
                        switch (atyp[k])
                        {
                    ...
                    {
                        string val = new string(encoding.GetChars(bf)).Trim();
                        switch (atyp[k])
                        {
                            case DbfColumn.DbfColumnType.Character:
                                bi.Append(@"""" + val + @"""");
        ...
        public static int ComputeAdditionChecksum(byte[] data)
        {
            int sum = 0;
